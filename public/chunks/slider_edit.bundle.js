  ...
  data: function data() {
    return {
      form: new vform__WEBPACK_IMPORTED_MODULE_1__["Form"]({
        image: "",
        file: this.$store.state.image_base_link,
        position: 1,
        url: "#"
      }),
      error: "",
      ...
      image_width: 870,
      image_height: 390,
      image_size_text: "Image size must be 870*390px",
      imagae_size: 550
    };
  },
  methods: {
    getEditSlider: function getEditSlider() {
      ...
      axios.get('/get/edit/slider/' + this.$route.params.id).then(function (resp) {
        if (resp.data.status == "OK") {
          ...
          _this.form.position = resp.data.slider.position;
          _this.form.file = _this.$store.state.image_base_link + resp.data.slider.image;
          _this.loading = false;
      ...
      reader.onload = function (evt) {
        ...
        img.onload = function () {
          ...
          if (img.width == _this3.image_width && img.height == _this3.image_height) {
            ...
            alert("Image size need" + _this3.image_width + "*" + _this3.image_height + "px . But Upload imaze size" + img.width + "*" + img.height + "px");
            return;
          }
        };
